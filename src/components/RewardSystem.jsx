...
const BADGES = [
  ...
  { id:"books",    icon:"📚", title:"Scholar",         desc:"Tracked a book/stationery expense",     unlocked:e=>e.some(x=>x.category?.toLowerCase().includes("book") || x.category?.toLowerCase().includes("edu") || x.category?.toLowerCase().includes("stat")), task:"Track an Education/Book expense" },
  { id:"health",   icon:"💊", title:"Health Aware",    desc:"Tracked a health expense",              unlocked:e=>e.some(x=>x.category?.toLowerCase().includes("health") || x.category?.toLowerCase().includes("med") || x.category?.toLowerCase().includes("fit")), task:"Track a Health/Medicine expense" },
  { id:"mindful",  icon:"🥗", title:"Mindful Spender", desc:"Food < 40% of total spending",          unlocked:e=>{ const t=e.reduce((s,x)=>s+x.amount,0); const f=e.filter(x=>x.category?.toLowerCase().includes("food") || x.category?.toLowerCase().includes("din")).reduce((s,x)=>s+x.amount,0); return t>0&&f/t<0.4; }, task:"Keep food spending below 40%" },
...
const RW = {
  ...
  profileCard:  { borderRadius: 20, padding: "20px 24px", backdropFilter: "blur(12px)" },
  profileIcon:  { width: 64, height: 64, borderRadius: "50%", border: "2px solid", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 30, flexShrink: 0, background: "var(--bg-card)", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" },
  ...
  calDay:       { width: 40, height: 40, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "transform 0.2s" },
  progressPill: { position: "relative", width: 84, height: 18, background: "var(--border)", borderRadius: 9, overflow: "hidden" },
  progressFill: { position: "absolute", left: 0, top: 0, height: "100%", background: "var(--green)", borderRadius: 9, transition: "width 0.8s" },
  progressText: { position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: "var(--text-primary)" },
  groupLabel:   { fontSize: 11, fontWeight: 800, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 10px" },
  badgeGrid:    { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(110px,1fr))", gap: 12 },
  badgeCard:    { background: "var(--bg-input)", border: "1px solid var(--border)", borderRadius: 14, padding: "16px 10px", textAlign: "center", transition: "transform 0.2s" },
  badgeTitle:   { margin: "0 0 4px", fontSize: 12, fontWeight: 800, color: "var(--text-primary)" },
  badgeDesc:    { margin: 0, fontSize: 10, color: "var(--text-muted)", lineHeight: 1.4, fontWeight: 500 },
