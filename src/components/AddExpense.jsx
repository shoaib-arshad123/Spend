...
import { getCategoryIcon, translations } from "../i18n/translations";
...
export default function AddExpense({ t, lang, onAdd, setActiveTab }) {
  ...
  const dateStr = today.toLocaleDateString("en-PK", { weekday:"short", year:"numeric", month:"short", day:"numeric" });
  ...
  const submit = () => {
    ...
    onAdd({ amount:Number(amount), category, description:note.trim(), date:isoDate });
    setSuccess(true); setAmount(""); setNote(""); setCategory(getAutoCategory()); setError("");
      ...
      <div style={AE.container}>
        ...
        <div style={AE.pageHeader}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <motion.button style={AE.scanBtn} onClick={() => setShowScan(true)} whileHover={{ scale:1.03 }}>
              📸 Scan Bill
            </motion.button>
          </div>
        </div>
        ...
        <div style={AE.timeRow}>
          <div style={AE.timePill}><span>📅</span>{dateStr}</div>
          <div style={AE.timePill}><span>⏰</span>{timeStr}</div>
          <div style={{ ...AE.timePill, color:"var(--green)", borderColor:"rgba(16,185,129,0.3)" }}>✓ Auto-detected</div>
        </div>
        ...
        <div style={AE.amountSection}>
          ...
          <div style={AE.quickRow}>
            {quickAmounts.map(q => (
              <button key={q} style={AE.quickBtn} onClick={() => setAmount(String(q))}>+{q}</button>
            ))}
          </div>
        ...
        <div style={AE.section}>
          ...
          <div style={AE.catGrid}>
            {(categories.length > 0 ? categories : [{name: 'Other', icon: '📌'}]).map(cat => {
              const active = category === cat.name;
              const color = DEFAULT_COLOR; // You can add colors to DB later
              return (
                <motion.button
                  key={cat.name}
                ...
                >
                  ...
                  <span style={{ fontSize:11, fontWeight: active ? 700 : 400 }}>{cat.name}</span>
        ...
        {/* Buttons */}
        <div style={AE.btnRow}>
          <button style={AE.cancelBtn} onClick={() => setActiveTab("dashboard")}>{t.cancel}</button>
